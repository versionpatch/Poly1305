...
int main(int argc, char* argv[])
{
    ...
    std::string input_tag = argv[3];
    if (input_tag.length() != 32)
    {
        std::cerr << "Tag must be 16 bytes written in hexadecimal." << std::endl;
        return -1;
    }
    std::string tag_s = reverse_endianness(input_tag);
    if (tag.set_str(tag_s, 16) != 0)
    {
        std::cerr << "Tag must be in hexadecimal" << std::endl;
        return -1;
    }
